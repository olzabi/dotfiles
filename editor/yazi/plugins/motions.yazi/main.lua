...
-- Compatibility shim: ya.emit was ya.mgr_emit before Yazi 0.4
local emit = ya.emit or ya.mgr_emit

-- Compatibility shim: trigger a UI re-render
local function trigger_render()
	if ui and ui.render then
		ui.render()
	elseif ya.render then
		ya.render()
	end
end

-- Compatibility shim: access theme. THEME replaced th in Yazi 0.4
...
local render_motion = ya.sync(function(_, motion_num, motion_cmd)
	...
	Status.motion = function(self)
		...
		-- Resolve background colour regardless of whether .bg is a field or method
		local bg_style
...
local render_numbers = ya.sync(function(_, mode)
	...
	Entity.number = function(_, index, total, file, hovered)
		...
		if hovered == index then
			return ui.Span(string.format(num_format .. " ", idx)):reset()
		else
			return ui.Span(string.format(" " .. num_format, idx)):reset()
		end
	end
...
local function get_cmd(first_char, keys)
	...
	while true do
		...
		-- ya.which returns an index (number) in older Yazi and a table/string in newer;
		-- normalise to the "on" string in both cases.
		if not key then
			return nil, nil, nil
		end
		...
		elseif type(key) == "table" then
			last_key = key.on or key[1]
		else
			last_key = tostring(key)
		end
	...
	if last_key == "g" or last_key == "v" or last_key == "d" or last_key == "y" or last_key == "x" then
		...
		if type(direction_key) == "number" then
			direction = DIRECTION_KEYS[direction_key].on
		elseif type(direction_key) == "table" then
			direction = direction_key.on or direction_key[1]
		else
			direction = tostring(direction_key)
		end
...
local get_cache_or_first_dir = ya.sync(function(state)
	...
	-- Fall back to first directory in the listing.
	-- cx.active.current.files may be unavailable in some builds; fall back to .window.
	local files = cx.active.current.files or cx.active.current.window
	local index = 1
